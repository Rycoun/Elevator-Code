...
public class BankAccountTest {
    ...
    public void test02_HappyPath_Tests() throws InvocationTargetException, IllegalAccessException, InstantiationException, NoSuchMethodException {
        ...
        assertEquals("BankAccount deposit method fails to increase balance by correct amount. Starting balance: 100, deposit: 23, new balance should be 123.", 123, getBalance.invoke(bankAccount));
        ...
        assertEquals("BankAccount withdraw method fails to decrease balance by correct amount. Starting balance: 123, withdraw: 22, new balance should be 101.", 101, getBalance.invoke(bankAccount));
    ...
    public void test03_EdgeCase_Tests() throws IllegalAccessException, InvocationTargetException, InstantiationException, NoSuchMethodException {
        ...
        assertEquals("BankAccount withdraw method fails to decrease balance by correct amount. Starting balance: 1, withdraw: 1, new balance should be 0.", 0, getBalance.invoke(bankAccount));

        // Assert deposit method can handle 0 balance
        bankAccount = threeArgConstructor.newInstance("", "", 0);
        Method deposit = bankAccount.getClass().getMethod("deposit", int.class);
        deposit.invoke(bankAccount, 1);
        assertEquals("BankAccount deposit method fails to increase balance by correct amount. Starting balance: 0, deposit: 1, new balance should be 1.", 1, getBalance.invoke(bankAccount));
        ...
        assertEquals("BankAccount deposit method fails to prevent negative deposit amount. Starting balance: 100, deposit: -10, balance should remain at 100.", 100, getBalance.invoke(bankAccount));
        ...
        assertEquals("BankAccount withdraw method fails to prevent negative withdraw amount. Starting balance: 100, withdraw: -10, balance should remain at 100.", 100, getBalance.invoke(bankAccount));
    }
