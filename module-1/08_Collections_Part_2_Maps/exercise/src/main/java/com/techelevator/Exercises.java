...
public class Exercises {
	...
	public String animalGroupName(String animalName) {
		...
		String moreNames  = animalName.toLowerCase();
		Map<String, String> groups = new HashMap<>();


		groups.put("rhino", "Crash");
		groups.put("giraffe", "Tower");
		groups.put("elephant", "Heard" );
		...
		return groups.getOrDefault(moreNames, "unknown");
	...
	public double isItOnSale(String itemNumber) {
		...
		String itemNum = itemNumber.toLowerCase();
		if (itemNum.startsWith("kitchen")) {
			return 0.20;
		} else if (itemNum.startsWith("garage")) {
			return 0.15;
		} else if (itemNum.startsWith("bath")) {
			return 0.15;
		} else if (itemNum.startsWith("bedroom")) {
			return 0.60;
		} else if (itemNum.startsWith("livingroom")) {
			return 0.10;
		} else {
			return 0.00;
		}
	...
	public Map<String, Integer> robPeterToPayPaul(Map<String, Integer> peterPaul) {
		if (peterPaul.containsKey("Peter") && peterPaul.containsKey("Paul")) {
			int peterMon = peterPaul.get("Peter");
			int paulMon = peterPaul.get("Paul");

			if (peterMon > 0 && paulMon < 1000) {
				int ammount = peterMon / 2;
				peterPaul.put("Peter", peterMon - ammount);
				peterPaul.put("Paul", paulMon + ammount);
			}
	...
	public Map<String, Integer> peterPaulPartnership(Map<String, Integer> peterPaul) {
		if (peterPaul.containsKey("Peter") && peterPaul.containsKey("Paul")) {
			...
			if (peterMon >= 5000 && paulMon >= 10000) {
				int cont = peterMon / 4;
				peterPaul.put("Peter", peterMon - cont);
				peterPaul.put("Paul", paulMon - cont);
				peterPaul.put("PeterPaulPartnership", cont);
	...
	public Map<String, String> beginningAndEnding(String[] words) {
		...
		for (String word : words) {
			if (!word.isEmpty()) {
				...
				total.put(String.valueOf(firstLetter), String.valueOf(lastLetter));
	...
	public Map<String, Boolean> wordMultiple(String[] words) {
		...
		for (String word : count.keySet()) {
			output.put(word, count.get(word) >= 2);
	...
	public Map<String, Integer> last2Revisited(String[] words) {
		...
		for (String word : words) {
			int counter = 0;
			if (word.length() >= 2) {
				String moreLast2 = word.substring(word.length() - 2);
				for (int i = 0; i < word.length() - 2; i++) {
					String fire = word.substring(i, i + 2);
					if (fire.equals(moreLast2)) {
						counter++;
					}

				}
			}
			last2.put(word, counter);
		}
