...
create table painting(
	painting_id serial,
	painting_title varchar(500) not null,
...
insert into painting (painting_title) values ('tech elevator campus');
insert into artist_painting (artist_id, painting_id)
values ( (select artist_id from artist where name = 'Bryce'), (select painting_id from painting where painting_title = 'tech elevator campus'));
...
values ( (select customer_id from customer where name = 'Walt'),
		(select painting_id from painting where painting_title = 'tech elevator campus'),
	    '2023-10-15', 1500.55);
