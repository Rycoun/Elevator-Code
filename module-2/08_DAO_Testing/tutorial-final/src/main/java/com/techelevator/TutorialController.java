    ...
    private void saleMenu() {
        ...
        while (!finished) {
            ...
            if (selection.equals(SELECT)) {
                ...
                // DAO in action !!!
                try {
                    sales = saleDao.getSalesByCustomerId(currentCustomer.getCustomerId());
                } catch (DaoException e) {
                    console.printErrorMessage("DAO error occurred: " + e.getMessage());
                }
                ...
                if (currentSale != null) {
                    if (view.deleteSale(currentSale)) {
                        // DAO in action !!!
                        try {
                            saleDao.deleteSaleById(currentSale.getSaleId());
                            console.printBlankLine();
