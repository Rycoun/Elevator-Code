		...
		try {
			...
			while (rsExpected.next() && rsActual.next()){
				for (String colNameExpected : colNamesExpected) {
					...
					int rowNumber = rsExpected.getRow();
					String message = "Data returned doesn't match data expected for row " + rowNumber + " in column '" + colNameExpected + "'.";
					if (rowNumber == 1) {
						message += " Check that your results are ordered correctly.";
					}
					assertEquals(message, valExpected, valActual);
				}
			}
		} catch (SQLException ex) {
			Assert.fail(ex.getMessage());
