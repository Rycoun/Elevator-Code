        ...
        describe('Part 1: Practice with flexbox properties', () => {
            ...
            describe('Set 6: Order flex items', () => {
                ...
                describe('Step 1: Order flex items', () => {
                    ...
                    it('The container of the first text-image pair must be a row-based flex container, and the image must precede the text.', () => {
                    ...
                    cy.get('section#left-panel').should('have.css', 'flex-basis').then((basis) => {
                        if (basis !== '35%') {
                            ...
                            cy.get('section#left-panel').should('have.css', 'width').then(panelWidth => {
                                if (panelWidth !== '35%') {
                                    let pxPanelWidth = parseInt(panelWidth);
                                    cy.get('body').should('have.css', 'width').then(bodyWidth => {
                                        let pxBodyWidth = parseInt(bodyWidth);
                                        let actualPercent = Math.round((pxPanelWidth * 100) / pxBodyWidth, 1);
                                        assert(Math.abs(actualPercent - 35) <= 2, `Left panel width ${pxPanelWidth} divided by body width ${pxBodyWidth} should be 35%, but it was calculated to be ${actualPercent}%. Switch to using flex-basis instead of width.`);
                                    });
                                }
                            });
                        }
