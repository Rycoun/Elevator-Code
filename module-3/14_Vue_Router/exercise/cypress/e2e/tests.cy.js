...
describe('Routing Exercise', () => {
  ...
  describe("Step Six Tests", () => {
    it('should show book details when loading book page', () => {
      cy.visit('/book/9781784877989');
      cy.contains('Kafka on the Shore');
      cy.contains('Haruki Murakami');
      cy.contains("505"); // Number of pages
      cy.get('a[href$="https://en.wikipedia.org/wiki/Kafka_on_the_Shore"]');
    });
    it('should link to book page from reading list cards', () => {
      cy.visit('/myBooks');
      cy.get('a[href$="/book/9781784877989"]');
