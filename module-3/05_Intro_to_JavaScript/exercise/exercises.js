...
function frontAgain (string) {
	if (string.substring(0, 2) == string.substring(string.length - 2)) {
		return true;
	}
...
10. **filterBigNumbers** Write a function that filters numbers greater than or equal to 100.
	...
	filterBigNumbers([]) → []
*/
function filterBigNumbers(numberArray) {
    return numberArray.filter(n => n >= 100);
}
...
*/
function filterMultiplesOfX(numberArray, parameterMultiple) {
    return numberArray.filter(n => n % parameterMultiple == 0);
}
