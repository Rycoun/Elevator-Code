...
describe('CSS Grid tests', () => {
    ...
    //
    // Note: Although the media query is for 450px and below, the test uses a 449 pixel viewport.
    //       Depending on the screen resolution and Windows text scale setting, a 450 pixel viewport 
    //       is sometimes created slightly larger than 450 pixels which causes these tests to fail
    //
    ...
    function checkRowsAndColumns(grid, expectedRows, expectedColumns) {
        assert(grid.length === expectedRows, `Expected ${expectedRows} rows in the grid area, but there were ${grid.length}.`);
        grid.forEach((cols) => {
